 ...
 * @author xudong.weng
 */
public class JDMirror extends javax.swing.JDialog implements Runnable{

    private Thread selfThread = null;
    
    public JDMirror(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        this.updateLink();
    ...
    private void jbnCompareStartActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbnCompareStartActionPerformed
        ...
        if(selfThread == null|| selfThread.getState().toString().equals("TERMINATED")) {
            ...
            selfThread = new Thread(JDMirror.this);
            selfThread.start();
        }else{
    ...
    private void dbCompare(){
        ...
        int stsa=MySQLHelper.getCon(msoA.getUser(), msoA.getPassword(), msoA.getServer(), msoA.getPort());

        
        if(stsa==1){
            sbmsg.append("Connection is successful");
            this.jtaCompareInfo.insert(sbmsg.append("\n").toString(), this.jtaCompareInfo.getText().length());
        }else{
            sbmsg.append("Connection is failture");
            this.jtaCompareInfo.insert(sbmsg.append("\n").toString(), this.jtaCompareInfo.getText().length());
            return;
        }
        ...
        int stsb=MySQLHelper.getCon(msoB.getUser(), msoB.getPassword(), msoB.getServer(), msoB.getPort());
        if(stsb==1){
            sbmsg.append("Connection is successful");
            this.jtaCompareInfo.insert(sbmsg.append("\n\n").toString(), this.jtaCompareInfo.getText().length());
        }else{
