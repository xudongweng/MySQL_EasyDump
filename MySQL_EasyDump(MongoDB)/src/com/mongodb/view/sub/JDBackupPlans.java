...
import com.xml.helper.UUIDHelper;
 ...
 * @author xudong.weng
 */
public class JDBackupPlans extends javax.swing.JDialog {
    ...
    public JDBackupPlans(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
    ...
    private void jbOKActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jbOKActionPerformed
        ...
        DatabaseDic databaseDic=LinkDic.getLink(linkname);
        if(databaseDic!=null&&databaseDic.getDatabases()!=null)
            databaseDic.clearAll();
        //}
        //查找是否存在需要备份的数据库
        if(dlmBackupList.getSize()>0){
            if(MongoDBController.delSubDocumentAll(linkname)<0){//删除子文档
                JOptionPane.showMessageDialog(null, "MongoDB delete connection is fault", "Error", JOptionPane.ERROR_MESSAGE);
            }
            if(MongoDBController.insertSubDocumentAll(linkname, dcTemp)<0){//插入子文档
                JOptionPane.showMessageDialog(null, "MongoDB insert connection is fault", "Error", JOptionPane.ERROR_MESSAGE);
            }
            for(int i = 0; i < dlmBackupList.getSize(); i++) {
                if(arrDBList==null){
            ...
            if(MongoDBController.delSubDocumentAll(linkname)<0){
                JOptionPane.showMessageDialog(null, "MongoDB delete connection is fault", "Error", JOptionPane.ERROR_MESSAGE);
            }
        }
