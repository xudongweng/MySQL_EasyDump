 ...
 */
package com.xml.controller.thread;
...
public class ZipThread extends Thread{
    ...
    @Override
     public void run() {
        ...
        sbLog.append(NowString.getTime("yyyy-MM-dd HH:mm:ss")).append(" Start compress：").append(this.zippath).append("\n");
        try{
            ZipUtilsHelper.compress(this.zippath);
        }catch(Exception e){
            FileHelper.outputFileFormat(PublicProperty.getErr(),"ZipThread.run",e.getMessage());
        }
        sbLog.append(NowString.getTime("yyyy-MM-dd HH:mm:ss")).append(" Finished compredd：").append(this.zippath).append("\n");
